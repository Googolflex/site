...
$queryCondition = "";
if (!empty($query)) {
    $queryCondition = "AND LOWER(name) LIKE ?";
}

$stmt = $pdo->prepare("SELECT * FROM products WHERE price BETWEEN ? AND ? $manufacturerCondition $productTypeCondition $queryCondition $orderBy");
